        ...
        (function (main) {
            var Main = (function (_super) {
                ...
                function Main(target) {
                    ...
                    values.main = _this;
                    // socket.io
                    _this._socket = new imjcart.logic.socket.SocketMain();
                    // 3D表示
                    _this._view3d = new main.view3d.View3d($("#view3d"));
                    // シーン
                    _this._scene = new main.scene.Scene($("#scene"));
                    // ランキング
                    _this._ranking = new main.ranking.Ranking($("#ranking"));
                    //
                    ...
                    //
                    // シーン変更イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.CHANGE_SCENE_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // タイムアタックシーン変更イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.CHANGE_TIMEATACK_SCENE_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // カラー変更イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.CHANGE_COLOR_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                        switch (evt.id) {
                    ...
                    // 車の描画更新イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.RENDER_CAR_EVENT, function (evt) {
                        // 車の姿勢更新
                    ...
                    // カメラアングル変更イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.CHANGE_CAMERA_ANGLE_EVENT, function (evt) {
                        // カメラアングル変更
                        _this._view3d.changeCameraAngle();
                    });
                    // 他の車追加イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.ADD_OTHER_CAR_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // 他の車削除イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.REMOVE_OTHER_CAR_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                        var arr = [];
                        var i = 0, max;
                    ...
                    // 他の車の描画更新イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.RENDER_OTHER_CAR_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                        var i = 0, max;
                    ...
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.EMIT_ID_FROM_SERVER_EVENT, function (evt) {
                    ...
                    // タイムアタック一時停止イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.PAUSE_TIMEATTACK_EVENT, function (evt) {
                        // タイムアタック一時停止
                        _this._scene.pauseTimeAttack();
                    });
                    // タイムアタック再開イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.RESUME_TIMEATTACK_EVENT, function (evt) {
                        // タイムアタック再開
                        _this._scene.resumeTimeAttack();
                    });
                    // タイムアタックリトライイベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.RETRY_TIMEATTACK_EVENT, function (evt) {
                        // タイムアタックリトライ
                        _this._scene.retryTimeAttack();
                    });
                    // ファステストラップ設定イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.SET_FASTEST_LAP_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                        values.fastestLapTime = evt.time;
                    });
                    // 名前設定イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.SET_NAME_EVENT, function (evt) {
                        var name = evt.name;
                        if (!name || name == "")
                            name = "No Name";
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // ラップタイム保存イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.SAVE_LAPTIME_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // ラップタイム保存完了イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.COMPLETE_SAVE_LAPTIME_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
                    ...
                    // ランキングデータリクエストイベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.GET_RANKING_FROM_CLIENT_EVENT, function (evt) {
                        // ランキングデータリクエスト
                        _this._socket.getRankingFromClient(evt.skip, evt.limit);
                    });
                    // ランキングデータ受信イベント
                    _this.addEventListener(imjcart.logic.event.ProjectEvent.GET_RANKING_FROM_SERVER_EVENT, function (evt) {
                        var values = imjcart.logic.value.GlobalValue.getInstance();
