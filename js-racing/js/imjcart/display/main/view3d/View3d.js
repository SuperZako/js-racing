            ...
            (function (view3d) {
                var View3d = (function (_super) {
                    ...
                    function View3d(target) {
                        ...
                        _this._carWheelAngle = null;
                        _this._carSpeed = null;
                        _this._cameraAngle = view3d.value.View3dConst.CAMERA_ANGLE_DEFAULT;
                        //
                        _this._ghostCarArr = [];
                        // シーン
                        ...
                        _this._scene.add(ambient);
                        // レンダラ―
                        //this._renderer = new THREE.WebGLRenderer();
                        _this._renderer = new THREE.WebGLRenderer({ antialias: true });
                        _this._renderer.setSize(window.innerWidth, window.innerHeight - imjcart.logic.value.Const.FOOTER_HEIGHT - imjcart.logic.value.Const.RANKING_HEIGHT);
                        _this._renderer.setClearColor(0x2077ab, 1);
                        if (imjcart.logic.value.Const.IS_SHADOW_ENABLED) {
                            _this._renderer.shadowMapEnabled = true;
                        }
                        _this.$target.append(_this._renderer.domElement);
                        // ライト
                        _this._light = new view3d.Light(_this._scene);
                        // カメラ
                        _this._camera = new view3d.Camera(_this._scene);
                        // 背景
                        _this._background = new view3d.Background(_this._scene);
                        // コース
                        _this._cource = new view3d.Cource(_this._scene);
                        // 車
                        _this._car = new view3d.Car(_this._scene);
                        // アイコン
                        _this._icon = new view3d.Icon(_this._scene);
                        //
                        ...
                        if (imjcart.logic.value.Const.IS_VIEW3D_DEBUG_MODE) {
                            ...
                            _this._axis = new THREE.AxisHelper(1000);
                            _this._axis.position.set(0, 0, 0);
                            _this._scene.add(_this._axis);
                            _this._controls = new THREE.OrbitControls(_this._camera.camera);
                        }
                        return _this;
                    }
